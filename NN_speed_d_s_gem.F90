...
subroutine nn_pred_1d_sgemm(x,y)
    ...
    ! local
    integer :: ilayer
    real(kind=4), dimension(:), allocatable :: interm
    ! for sgemm
    integer :: m, k, n, i, j
    ! num_layers matmul called
    m = 1
...
subroutine nn_pred_1d_dgemv(x,y)
    real(kind=8), dimension(:), intent(in) :: x
    real(kind=8), dimension(:), intent(inout) :: y
    ! local
    real(kind=8), dimension(:), allocatable :: interm
    ! for sgemm
    integer :: m, n
    integer :: ilayer
    m = Rad_NN_FC%num_hid_nodes
    ...
    do ilayer = 1, Rad_NN_FC%num_layers
        y = Rad_NN_FC%Layers(ilayer)%bias
        call DGEMV('T',m,n,1.0,Rad_NN_FC%Layers(ilayer)%weight,m,interm,1,1.0,y,1)
        y = max(y,0.0)
        interm = y
    end do
    deallocate(interm)
end subroutine nn_pred_1d_dgemv
...
allocate(ad(num_hid_nodes, total_run))
call random_seed()
...
call cpu_time(start)
do j = 1, total_run
    allocate(cd(size(ad,1)))
    call nn_pred_1d_dgemv(ad(:,j),cd)
    if (j==1 .and. check) then ; print *,cd(:2); end if
    deallocate(cd)
end do
call cpu_time(finish)
print '("DGEMV  avg time per run = ",f6.3," ms")', (finish-start)/total_run*1e3
End Program test
